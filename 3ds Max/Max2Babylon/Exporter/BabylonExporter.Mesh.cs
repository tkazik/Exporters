...
namespace Max2Babylon
{
    partial class BabylonExporter
    {
        ...
        int CreateGlobalVertex(IIGameNode meshNode, IIGameMesh mesh, BabylonAbstractMesh babylonAbstractMesh, IMatrix3 invertedWorldMatrix, IFaceEx face, int facePart, List<GlobalVertex> vertices, bool hasUV, bool hasUV2, bool hasColor, bool hasAlpha, List<GlobalVertex>[] verticesAlreadyExported, IIGameSkin skin, List<int> boneIds)
        {
            ...
            //System.Diagnostics.Debug.WriteLine("vertex normal world: " + string.Join(", ", mesh.GetNormal((int)face.Norm[facePart], false).ToArray().Select(v => Math.Round(v, 3))));
            //System.Diagnostics.Debug.WriteLine("vertex normal object: " + string.Join(", ", mesh.GetNormal((int)face.Norm[facePart], true).ToArray().Select(v => Math.Round(v, 3))));


            IPoint3 objOffsetPos = meshNode.MaxNode.ObjOffsetPos;
            IQuat objOffsetQuat = meshNode.MaxNode.ObjOffsetRot;
            IPoint3 objOffsetScale = meshNode.MaxNode.ObjOffsetScale.S;
            ...
            // in maxscript: offsetTM = (scaleMatrix $.objectOffsetScale) * ($.objectOffsetRot as matrix3) * (transMatrix $.objectOffsetPos)
            IMatrix3 offsetTM = Tools.Identity;
            offsetTM.Scale(objOffsetScale, false);
            offsetTM.MultiplyBy(objOffsetRotMat);
            offsetTM.Translate(objOffsetPos); // would not be needed for normal (only rotation and scale important)

            // apply offset TM to normal in object space to get normal in local/node space
            IPoint3 normalLocal = offsetTM.VectorTransform(vertex.Normal);
            ...
            vertex.Normal = normalLocal.Normalize;
